...
class Constraint:
    def __init__(
        ...
        self._pseudo_attribs = {'length', 'position', 'leftpos', 'rightpos'} # pseudo-attributes are computed
        self.type = type
        self._query = query
    ...
    def _get_layer_info(self, conf: Config, layer: str) -> JSONObject:
        ...
        assert layer in conf.config["layer"], ReferenceError(f"No entity type named {layer} in this corpus")
        info = cast(dict, conf.config["layer"][layer])

        if conf.lang:
            lginfo: dict = cast(dict, conf.config["layer"].get(f"{layer}@{conf.lang}", {}).get("attributes", {}))
    ...
    def _get_cast(self, field: str, query: Any) -> str:
        ...
        if self._query_variable:
            return ""
        if self._is_meta:
            met = cast(dict, self._layer_info.get("meta",self._attribs.get("meta", {})))
    ...
    def make(self) -> None:
        ...
        field_info = self._attribs.get(field, self._attribs.get("meta",{}).get(field, {}))
        ...
        if "." in self.field:
            ...
            if field_ref:
                global_attribute = self.config.get('globalAttributes',{}).get(field_ref)
        ...
        if field_type == "labels" or self.op.lower().endswith("contain"):
            ...
            assert self.type in ("string","regex"), TypeError("Contained labels can only be tested against strings or regexes")
            assert "nlabels" in self._layer_info, KeyError(f"Missing 'nlabels' for layer '{self.layer}' in config")
        ...
        if alignment := self.config['mapping']['layer'].get(self.layer,{}).get("alignment",{}).get("relation",{}):
    ...
    def parse_field_function(self, attributes_mapping: dict[str,Any]) -> str:
        ...
        assert fn in ('length','century','decade','year','month','day'), NotImplementedError(f"Function '{fn}' not implemented (yet?)")
        ars = self.field_function.get("arguments", [])
        assert ars, ValueError(f"Arguments of function '{fn}' are missing")
        assert "function" not in ars[0], NotImplementedError("Nested functions are not supported yet")
        ...
        if '.' in single_arg:
            ...
            arg_type = self._attribs[arg_field].get("type", self._attribs[arg_field].get("meta",{}).get("type"))
            if self._attribs[arg_field].get("ref"):
                arg_type = self.config.get('globalAttributes',{}).get(arg_field,{}).get("type")
    ...
    def parse_entity(self, field_type, relational_field) -> dict[str,Any]:
        ...
        entity_field_info = self.config['layer'][entity_layer].get("attributes",{}).get(entity_field)
        ...
        if entity_field_ref := entity_field_info.get("ref"):
            entity_global_attribute = self.config.get('globalAttributes',{}).get(entity_field_ref)
        ...
        # )
        entity_field_mapping = _get_mapping(
            entity_layer,
            self.config,
            self.batch,
            cast(str, self.lang)
    ...
    def metadata(self):
        ...
        if (
            ...
            self._add_join_on(f"{self.schema}.{relation} {lab}", f"{self.label}.alignment_id = {lab}.alignment_id")
    ...
    def deprel(self):
        ...
        source, target = {x.get("name","") for n, x in self._attribs.items() if n in ("source","target")}
        ...
        if field in (source, target):
            ...
            formed_condition = f"{label}.{field} {self.op} {entity_label}.{entity_layer}_id"
            self._add_join_on(
                f"{self.schema}.{dep_table} {label}",
                formed_condition.lower()
            )
        else:
...
def _get_constraints(
    ...
    if part_of:
        ...
        if part_of == label:
            part_of = ""
        elif is_token and part_of_layer.lower() == segname: # Use id for token in segment
