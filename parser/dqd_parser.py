...
assert os.path.isfile(dqd_grammar), f"Could not find a valid lark file in the current directory"
dqd_grammar = open(dqd_grammar).read()
json_schema = next((f for f in os.listdir() if f.endswith(".json")), "")
assert os.path.isfile(json_schema), f"Could not find a valid json file in the current directory"
...
def forward(schema: dict, label="") -> tuple[dict,bool]:
    ...
    while "properties" not in schema:
        if "items" in schema:
            ...
            schema = json_schema.get("$defs",{}).get(re.sub(r".+\/","",schema['$ref']))
...
def to_dict(tree: Any, properties_parent: dict={}) -> Any:
    ...
    schema, is_parent_array = forward(properties_parent.get(camel_name,properties_parent), label=camel_name)
    ...
    for child in tree.children:
        ...
        if child_name not in children_properties:
            fallback_name = to_camel( next((pn for pn,pv in schema.get("properties",schema).items() if found_rule_down_the_line(pv,child.data)), "") )
            if fallback_name:
    ...
    if skip_children_names and is_parent_array and all(isinstance(v,list) for v in list_values):
        assert len(list_values) == 1, f"Only one child list (<>{list_values}) can be embedded in a parent list ({camel_name})"
