...
class Ref():
    def __init__(self,name,options={}):
        self.name = name
        self.minItems = int(options.get("minItems")) if options.get("minItems") else None
        self.maxItems = int(options.get("maxItems")) if options.get("maxItems") else None
...
class Rule():
    ...
    def as_object(self):
        ...
        if len(self.references) > 1: # Big disjunction -- assumption: all disjuncts are named rules
            if self.name.endswith("__"): # If this is a forwarding rule, the named rules are properties
                for r in self.references:
                    ...
                    self._object['properties'][r[0].propertyName] = {"type":r[0].type, "$ref":f"#/$defs/{r[0].name}"}
                ...
                self._object['oneOf'] = [{"type":r[0].type, "$ref":f"#/$defs/{r[0].name}"} for r in self.references if isinstance(r[0],Ref)]
        else:
            ...
            if isinstance(refs,Ref): # only rules that point to a terminal have them as their direct, only reference child
                ...
                if len(refs)==1:
                    if isinstance(refs[0],list) and all(isinstance(x,Ref) for x in refs[0]):
                        ...
                        if required and not refs[0].name.endswith("__") and refs[0].type not in ("string","array"):
                    ...
                    for ref in refs:
                        if isinstance(ref,list): # Disjunction inside properties
                            ...
                            if "properties" in o: # Forwarding references will themselves return properties
                                ...
                                print(f"adding property {ref.propertyName} to {self.name}", ref, o)
        ...
        for p in ('oneOf','required','properties'):
            if p in self._object and not self._object[p]: self._object.pop(p)
        return self._object
    ...
    def type(self):
        ...
        if len(self.references) > 1:    # big disjunction
            ...
            if isinstance(refs, Ref) and (isinstance(refs, TermRef) or refs.name.endswith("__")):
                return refs.type
            elif len(refs)==1:
