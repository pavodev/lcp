...
The importer expects a file named `global_attribute_{KEY}.tsv` for each global attribute referenced under `globalAttributes`, with two columns, `{key}_id` and `{key}`, the latter of which should be a jsonb object whose keys correspond to the ones listed the global attributes `keys` entry. The TSV file of each annotation layer that refereces the global attribute should have a column named `{key}_id` that cross-references that column in `global_attribute_{KEY}.tsv`
...
The token layer requires at least three files named `{token}.tsv`, `{token}_form.tsv` and `{token}_lemma.tsv`, where `{token}` is the name associated with `token` in `firstClass` in the configuration file

`{token}.tsv` should have at least five columns:
 - `{token}_id` which should be an incremental integer
 - `form_id` which should be an integer index to look up `{token}_form.tsv`
 - `lemma_id` which should be an integer index to look up `{token}_lemma.tsv`
 - `{segment}_id` which should be UUIDs to look up `{segment}.tsv`, where `segment` is the value of `segment` in `firstClass`
...
`{token}_form.tsv` and `{token}_lemma.tsv` are simple lookup tables with two columns `form_id`/`lemma_id` and `form`/`lemma`, the former being an incremental integer index and the latter the corresponding text value
