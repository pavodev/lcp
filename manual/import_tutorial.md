...
Note that none of the Python scripts in this tutorial require the video files, so you can decide to only download the .src files. The video files will only be requires at the end of part 2, in order to successfully upload the data to _videoScope_.
...
Our approach will be as follows: each transcript file will correspond to a document, and each sequence of characters separated by a whitespace, a linebreak or single quote character will be modeled as a token; we will use periods, exclamation and question marks as delimiters for segments and ignore other punctuation signs. We intentionally keep things simple in this tutorial, although such an approach produces unusual outputs, such as `shouldn` and `t` being modeled as two separate tokens, as evident below.

Take this short sample of *database_explorer.srt* as an example:
...
What we need to achieve in this part of the tutorial is model the text above as follows:

```
character:  1    3         10      52       58       64    67

Token:      [It] [shouldn] [t] ... [speech] [corpus] [And] [luckily] ...
            1    2         3       14       15       16    17
...
Note that we have decided to ignore the lines that mark the block numbers (`1` and `2`) from the original transcript, as well as the timecodes (for now). The model above numbers each unit on a dedicated line under the `[` characters: the last of these lines numbers 1 document, the second one numbers 2 segments and the first one numbers 17 our of the 22 tokens in the sample (only seven of which are actually reported -- tokens 4 through 13 were elided). The line at the top numbers the *characters* in the sequence of tokens (ignoring the characters we chose to discard, namely the punctuation characters): the first token starts on the `1`st character and ends before the `3`rd character (i.e. it is 2-character long), the second token starts on the `3`r character and ends before the `10`th character (i.e. it is 7 characters long) and so on. Correspondingly, the first segment starts on the `1`st character and ends before the `64`th character, the second segments starts on the `64`th character and ends on the `100`th character (not shown here).
