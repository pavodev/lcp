...
LCP requires all corpora to define at least one flat character stream. More specifically, incoming corpora are expected to have purely string-based, sequential units. Take as an example corpus all the articles of the Universal Declaration of Human Rights: in that corpus, the words (understood as a space-character-separated string of text) are the best candidate for that unit, since each word is a simple string of characters and immediately precedes and/or follows another word. These properties make it possible for LCP users to run simple, standard queries, such as looking for the text "every individual" or looking for a sequence "every `NOUN`" where `NOUN` is meant to match any word annotated as such in the corpus. More concretely, in LCP, each corpus has a level of representation for the word, which needs to at least be associated with purely string-based representation (but which can also optionally be enriched with additional annotations, such as a lemmatized representation or a part-of-speech label, for example); LCP additionally models those words as totally linearly ordered, and reserves a level of representation for sequences of words (which can be made to correspond to sentences, for example) which can themselves contain additional annotations beside the exact sequence of basic units they contain.

LCP **cannot** straightforwardly accommodate corpora whose basic units have a more complex structure: for example, if an incoming corpus defines a basic unit "morpheme" that is recursive (say in XML `<morph form='declaration' pos='noun'><morph form='declare' pos='verb' type='root' /><morph form='ation' pos='noun' type='suffix' ></morph>`) then the corpus curator first needs to re-process their corpus to define a level of representation that LCP can immediately identify as the basic word unit.

Similarly, LCP **cannot** straightforwardly accommodate corpora whose basic units do not define a total linear order (at least within their immediate sentence-like context): for example, if the incoming corpus models occurrences of the English modal `can't` as `[ca[n]'t]` whereby the first three caracters `can` belong to one word-like unit (lemmatized as `can`) and the three last character `n't` belong to another word-like unit (lemmatized as `not`) then the corpus curator first needs to re-process their corpus to have non-overalpping units (`can` followed by `not`) which can be referenced in a parallel level of representation for multi-word units and reported with the merged form `can't`.
